...
Em uma instalação nova, aplique `sql/009-add-game-library-plugins.sql`. Em instalações que já possuem os plugins, aplique também `sql/010-add-monster-bestiary-categories.sql`. Depois, limpe o cache do MyAAC e use os botões administrativos nas páginas de monstros e magias para refazer a carga.
